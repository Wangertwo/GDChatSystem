...
{
    auto self_info = GDUserMgr::GetInstance()->GetUserInfo();
    ChatRole role;
    //todo... 添加聊天显示
    if (msg->_from_uid == self_info->_uid) {
        ...
        role = ChatRole::Other;
        ChatItemBase* pChatItem = new ChatItemBase(role);
        auto friend_info = GDUserMgr::GetInstance()->GetFriendById(msg->_from_uid);
...
void ChatPage::on_send_btn_clicked()
{
    ...
    auto user_info = GDUserMgr::GetInstance()->GetUserInfo();
    auto pTextEdit = ui->chatEdit;
    ChatRole role = ChatRole::Self;
    QString userName = user_info->_name;
    qDebug() << "user_info->_icon" << user_info->_icon;
    ...
    for(int i=0; i<msgList.size(); ++i)
    {
        ...
        if(type == "text")
        {
            ...
            if(txt_size + msgList[i].content.length()> 1024){
                ...
                textObj = QJsonObject();
                //发送tcp请求给chat server
                emit GDTcpMgr::GetInstance()->sig_send_data(ReqID::ID_TEXT_CHAT_MSG_REQ, jsonData);
            }
