    ...
    ui(new Ui::ApplyFriend),_label_point(2,6)
{
    ...
    InitTipLbs();
    //链接输入标签回车事件
    connect(ui->lb_ed, &GDCustomizeEdit::returnPressed, this, &ApplyFriend::SlotLabelEnter);
    connect(ui->lb_ed, &GDCustomizeEdit::textChanged, this, &ApplyFriend::SlotLabelTextChange);
    connect(ui->lb_ed, &GDCustomizeEdit::editingFinished, this, &ApplyFriend::SlotLabelEditFinished);
...
void ApplyFriend::InitTipLbs()
{
    ...
    for(int i = 0; i < _tip_data.size(); i++){
        // 创建Label
        auto* lb = new GDClickedLabel(ui->lb_list);
        // 设置属性
        ...
        // 绑定信号
        connect(lb, &GDClickedLabel::clicked, this, &ApplyFriend::SlotChangeFriendLabelByTip);
        // 计算宽高
...
void ApplyFriend::AddTipLbs(GDClickedLabel* lb, QPoint cur_point, QPoint& next_point, int text_width, int text_height)
{
    lb->move(cur_point);
    lb->show();
...
{
    _si = si;
    auto applyname = GDUserMgr::GetInstance()->GetName();
...
void ApplyFriend::resetLabels()
{
    ...
    for(auto it = _friend_labels.begin(); it != _friend_labels.end(); it++) {
        if(it.value()->Width() + _label_point.x() > max_width) {
            _label_point.setX(2);
            _label_point.setY(_label_point.y() + it.value()->Height() + 2);
        }

        it.value()->move(_label_point);
        it.value()->show();

        _label_point.setX(_label_point.x() + it.value()->Width() + 5);
        label_height = it.value()->Height();
    }

    if(_label_point.x() + MIN_APPLY_LABEL_ED_LEN > max_width) {
        ui->lb_ed->move(2, _label_point.y() + label_height + 2);
    }
...
void ApplyFriend::addLabel(QString name)
{
    ...
    int max_width = ui->gridWidget->width();
    if(tmpLabel->width() + _label_point.x() > max_width) {
        _label_point.setX(2);
        _label_point.setY(_label_point.y() + tmpLabel->Height() + 2);
    }
...
void ApplyFriend::SlotRemoveFriendLabel(QString name)
{
    ...
    auto it = std::find(_friend_label_keys.begin(), _friend_label_keys.end(), name);
    if(it != _friend_label_keys.end()) {
        _friend_label_keys.erase(it);
    }

    resetLabels();

    auto add_it = _add_labels.find(name);
    if(add_it == _add_labels.end()) {
        return;
    }

    add_it.value()->ResetNormalState();
}
...
void ApplyFriend::SlotApplySure()
{
    ...
    //发送请求逻辑
    QJsonObject jsonObj;
    auto uid = GDUserMgr::GetInstance()->GetUid();
    jsonObj["uid"] = uid;
    ...
    //发送tcp请求给chat server
    emit GDTcpMgr::GetInstance()->sig_send_data(ReqID::ID_ADD_FRIEND_REQ, jsonData);
    this->hide();
    deleteLater();
}
