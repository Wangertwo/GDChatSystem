    ...
    ui(new Ui::AuthenFriend),_label_point(2,6)
{
    ...
    InitTipLbs();
    //链接输入标签回车事件
    connect(ui->lb_ed, &GDCustomizeEdit::returnPressed, this, &AuthenFriend::SlotLabelEnter);
    connect(ui->lb_ed, &GDCustomizeEdit::textChanged, this, &AuthenFriend::SlotLabelTextChange);
    connect(ui->lb_ed, &GDCustomizeEdit::editingFinished, this, &AuthenFriend::SlotLabelEditFinished);
...
void AuthenFriend::InitTipLbs()
{
    ...
    for(int i = 0; i < _tip_data.size(); i++){
        ...
        connect(lb, &GDClickedLabel::clicked, this, &AuthenFriend::SlotChangeFriendLabelByTip);
...
void AuthenFriend::AddTipLbs(GDClickedLabel* lb, QPoint cur_point, QPoint& next_point, int text_width, int text_height)
{
    lb->move(cur_point);
    lb->show();
...
void AuthenFriend::SlotApplySure()
{
    ...
    //发送tcp请求给chat server
    emit GDTcpMgr::GetInstance()->sig_send_data(ReqID::ID_AUTH_FRIEND_REQ, jsonData);
