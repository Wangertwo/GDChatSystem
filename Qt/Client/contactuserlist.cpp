    ...
    ,_load_pending(false)
{
    ...
    //链接对端同意认证后通知的信号
    connect(GDTcpMgr::GetInstance().get(), &GDTcpMgr::sig_add_auth_friend,this,
...
void ContactUserList::addContactUserList()
{
    ...
    //加载后端发送过来的好友列表
    auto con_list = GDUserMgr::GetInstance()->GetConListPerPage();
...
bool ContactUserList::eventFilter(QObject *watched, QEvent *event)
{
    ...
    if (watched == this->viewport() && event->type() == QEvent::Wheel) {
        ...
        if (maxScrollValue - currentValue <= 0) {
            ...
            auto b_loaded = GDUserMgr::GetInstance()->IsLoadChatFin();
            if(b_loaded){
                return true;
            }
...
void ContactUserList::slot_add_auth_firend(std::shared_ptr<AuthInfo> auth_info)
{
    ...
    bool isFriend = GDUserMgr::GetInstance()->CheckFriendById(auth_info->_uid);
    if(isFriend){
        return;
    }
    // 在 groupitem 之后插入新项
...
void ContactUserList::slot_auth_rsp(std::shared_ptr<AuthRsp> auth_rsp)
{
    ...
    bool isFriend = GDUserMgr::GetInstance()->CheckFriendById(auth_rsp->_uid);
    if(isFriend){
        return;
    }
    // 在 groupitem 之后插入新项
